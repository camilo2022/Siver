...
use App\Exports\ConfeccionStaraExport;
...
class ConfeccionStaraController extends Controller
{
    public function download_excel()
    {
        $confeccion = DB::select('select id,fecha,turno,modulo,(SELECT referencias_stara.lote_referencia from referencias_stara WHERE modulos_stara.id_referencia=referencias_stara.id) AS referencia,tallas,tc, tipo, hora, cantidad,tiempo_req_h,meta_produccion,eficiencia,n_operarios,(SELECT paradas_programadas.tipo_parada_prg from paradas_programadas WHERE modulos_stara.id_parada_prg=paradas_programadas.id) AS parada_programada, (SELECT paradas_programadas.tiempo from paradas_programadas WHERE modulos_stara.id_parada_prg = paradas_programadas.id) AS tiempo_parada_programada, (SELECT paradas_no_programadas.tipo_parada_noprg from paradas_no_programadas WHERE modulos_stara.id_parada_noprg=paradas_no_programadas.id) AS parada_no_programada,tiempo_noprg from modulos_stara');
        return Excel::download(new ConfeccionStaraExport($confeccion),"ConfeccionStara.xlsx");
    }
    
    public function index_modulos()
    {
        $modulos = DB::select('select id,fecha,turno,modulo,(SELECT referencias_stara.lote_referencia from referencias_stara WHERE modulos_stara.id_referencia=referencias_stara.id) AS referencia,tallas,tc, tipo, hora, cantidad,tiempo_req_h,meta_produccion,eficiencia,n_operarios,(SELECT paradas_programadas.tipo_parada_prg from paradas_programadas WHERE modulos_stara.id_parada_prg=paradas_programadas.id) AS parada_programada, (SELECT paradas_programadas.tiempo from paradas_programadas WHERE modulos_stara.id_parada_prg = paradas_programadas.id) AS tiempo_parada_programada, (SELECT paradas_no_programadas.tipo_parada_noprg from paradas_no_programadas WHERE modulos_stara.id_parada_noprg=paradas_no_programadas.id) AS parada_no_programada,tiempo_noprg from modulos_stara where fecha BETWEEN date_add(NOW(), INTERVAL -15 DAY) AND NOW()');
        return view('horas_stara.tabla_modulos')->with('modulos',$modulos);
    }
    ...
    public function index_modulos_hoy()
    {
        ...
        $modulos = DB::select('select id,fecha,turno,modulo,(SELECT referencias_stara.lote_referencia from referencias_stara WHERE modulos_stara.id_referencia=referencias_stara.id) AS referencia,tallas,tc, tipo, hora, cantidad,tiempo_req_h,meta_produccion,eficiencia,n_operarios,(SELECT paradas_programadas.tipo_parada_prg from paradas_programadas WHERE modulos_stara.id_parada_prg=paradas_programadas.id) AS parada_programada, (SELECT paradas_programadas.tiempo from paradas_programadas WHERE modulos_stara.id_parada_prg = paradas_programadas.id) AS tiempo_parada_programada, (SELECT paradas_no_programadas.tipo_parada_noprg from paradas_no_programadas WHERE modulos_stara.id_parada_noprg=paradas_no_programadas.id) AS parada_no_programada,tiempo_noprg from modulos_stara where fecha = "'.$fecha.'"');
        return view('horas_stara.tabla_modulos')->with('modulos',$modulos);
    }
    ...
    public function create_modulos()
    {
        ...
        $referencias = DB::table('referencias_stara')->get();
        return view('horas_stara.form_modulos',compact('programadas','no_programadas','referencias'));
    }
    ...
    public function store_modulos(Request $request)
    {
        ...
        $cant_refe = DB::table('referencias_stara')->where('id','=',$id_refe)->get();
        $cant_ref = json_decode($cant_refe,true);
        $new_cant = $cant_ref[0]['cantidad_disponible_confeccion']-$cantidad;

        $validar_si_existe = DB::table('modulos_stara')->where('fecha','=',$fecha)->where('turno','=',$turno)->where('modulo','=',$modulo)->where('hora','=',$hora)->get();
    ...
    public function edit_modulos($id)
    {
        $programadas = DB::table('paradas_programadas')->get();
        $no_programadas = DB::table('paradas_no_programadas')->get();
        $modulos = DB::table('modulos_stara')->where('id','=',$id)->get();
        $modulo = json_decode($modulos,true);
        $referencias = DB::table('referencias_stara')->get();
        
        return view('horas_stara.edit_modulos',compact('programadas','no_programadas','modulo','referencias'));
    }
    ...
    public function update_modulos(Request $request, $id)
    {
        ...
        $updated_refe = Carbon::now();
        $cant_mod = DB::table('modulos_stara')->where('id','=',$id)->get();
        $can_mod = json_decode($cant_mod,true);
        $cant_refe = DB::table('referencias_stara')->where('id','=',$id_refe)->get();
        $cant_ref = json_decode($cant_refe,true);
        $cant_refe_new = DB::table('referencias_stara')->where('id','=',$can_mod[0]['id_referencia'])->get();
        $cant_ref_new = json_decode($cant_refe_new,true);
        $new_cant = 0;
        $new_cant_new = 0;
        
        $validar_si_existe = DB::table('modulos_stara')->where('fecha','=',$fecha)->where('turno','=',$turno)->where('modulo','=',$modulo)->where('hora','=',$hora)->where('id','!=',$id)->get();
    ...
    {
        $modulo = DB::table('modulos_stara')->where('id','=',$id)->get();
        $refe = DB::table('referencias_stara')->where('id','=',$modulo[0]->id_referencia)->get();
        DB::table('referencias_stara')->where('id','=',$modulo[0]->id_referencia)->update(['cantidad_disponible_confeccion'=>$refe[0]->cantidad_disponible_confeccion+$modulo[0]->cantidad]);
        DB::table('modulos_stara')->where('id','=',$id)->delete();
        return redirect()->route('list_modulos_hoy');
    }

    public function eficiencia($id)
    {
        $consulta_fecha = DB::table('modulos_stara')->where('id','=',$id)->get();
        $consulta = json_decode($consulta_fecha,true);
        $eficiencia_hoy = DB::table('modulos_stara')->where('fecha','=',$consulta[0]['fecha'])->where('turno','=',$consulta[0]['turno'])->get();
    ...
    public function store_referencias(Request $request)
    {
        ...
        $validar_refe = DB::table('referencias_stara')->where('lote_referencia','=',$lote)->get();
        if(count($validar_refe)>0){
            return response()->json(1);
        }else{
        DB::insert('insert into referencias_stara (referencia,lote_referencia,cantidad_lote,cantidad_disponible_confeccion,tc,created_at) values (?,?,?,?,?,?)', [$refe,$lote,$cant_lote,$cant_conf,$tc_refe,$created_at]);
        
        $new = DB::table('referencias_stara')->where('lote_referencia','=',$lote)->get();
        $array = json_decode($new,true);
        return response()->json($array);
        }
    }
    ...
    {
        $referencias = DB::table('referencias_stara')->where('id','=',$id)->get();
        $referencia = json_decode($referencias,true);
        return view('horas_stara.edit_referencia')->with('referencia',$referencia);
    }
    ...
    public function update_referencias(Request $request, $id)
    {
        ...
        $tc_refe = $request->get('tc_refe');
        $validar_refe = DB::table('referencias_stara')->where('lote_referencia','=',$lote)->where('id','!=',$id)->get();
        if(count($validar_refe)>0){
            return response()->json(2);
        }else{
        DB::table('referencias_stara')->where('id','=',$id)->update(['referencia'=>$refe,'lote_referencia'=>$lote,'cantidad_lote'=>$cant_lote,'cantidad_disponible_confeccion'=>$cant_conf,'tc'=>$tc_refe,'updated_at'=>$updated_at]);    
        return response()->json(1);
        }
    }
    ...
    public function validar_referencias(Request $request)
    {
        ...
        $validar = DB::table('referencias_stara')->where('lote_referencia','=',$referencia)->get();
    ...
    public function consultar_lote(Request $request)
    {
        $cantidad = $request->get('cantidad');
        $id_refe = $request->get('id_refe');
        $consulta_refe = DB::table('referencias_stara')->where('id','=',$id_refe)->get();
