...
<table id="mydatatable-container" class="table" style="width:100%">
    ...
    <tfoot>
        <tr>
            ...
            <td><input type="text" class="form-control long input-sm" placeholder="Fecha PT"></td>
            <td><input type="text" class="form-control long input-sm" placeholder="Usuario PT"></td>
            <td><input type="text" class="form-control long input-sm" placeholder="Observacion T"></td>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{$orden->fecha_picking_terminacion}}</td>
        <td>{{$orden->user_name_terminacion." ".$orden->user_apellido_terminacion}}</td>
        <td>{{$orden->observacion_terminacion}}</td>
        ...
        <td>
          ...
          @elseif($orden->status == "Enviado")  
            ...
            <a type="button" class="" data-toggle="modal" data-target="#modal-terminacion" onClick="datosTerminacion('{{$orden->id}}');" style="cursor: pointer;"><span class="badge badge-info"><i class="fa fa-solid fa-arrow-up"></i></span></a>
          @elseif($orden->status == "Aceptado")
            <a type="button" class="" data-toggle="modal" data-target="#modal-terminacion" onClick="datosTerminacion('{{$orden->id}}');" style="cursor: pointer;"><span class="badge badge-success"><i class="fa fa-solid fa-arrow-up"></i></span></a>
            <a type="button" class="" data-toggle="modal" data-target="#modal-bodega" onClick="datosBodega('{{$orden->id}}');" style="cursor: pointer;"><span class="badge badge-success"><i class="fa fa-solid fa-arrow-down"></i></span></a>           
          @elseif($orden->status == "En espera")
            <a href="{{route('revision_picking_bodega',$orden->id)}}"><span class="badge badge-warning"><i class="fa fa-solid fa-retweet"></i></span></a>
          @elseif($orden->status == "Rechazado")
            <a href="{{route('repicking_orden_picking',$orden->id)}}"><span class="badge badge-danger"><i class="fa fa-solid fa-paper-plane"></i></span></a>
            <a type="button" class="" data-toggle="modal" data-target="#modal-terminacion" onClick="datosTerminacion('{{$orden->id}}');" style="cursor: pointer;"><span class="badge badge-danger"><i class="fa fa-solid fa-arrow-up"></i></span></a>
            <a type="button" class="" data-toggle="modal" data-target="#modal-bodega" onClick="datosBodega('{{$orden->id}}');" style="cursor: pointer;"><span class="badge badge-danger"><i class="fa fa-solid fa-arrow-down"></i></span></a>
          @endif
        </td>
    </tr>
      @endforeach
    ...
    <div class="modal fade bd-example-modal-lg" id="modal-terminacion" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
      <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
              <div class="modal-header" style="background-color:#0058ff; color:white;">
                  <h5 class="modal-title" id="exampleModalLongTitle">Registros Terminacion</h5>
              ...
              <div class="modal-body" >
                  <div class="card-body">
                      <div class="table-responsive">    
                          <table id="mydatatable_terminacion" class="table" style="margin: auto;">
                              <thead class="">
                                  <tr>
                                      <th scope="col">Id</th>
                                      <th scope="col">Orden Picking</th>
  ...
  <div class="modal fade bd-example-modal-lg" id="modal-bodega" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header" style="background-color:#0058ff; color:white;">
                <h5 class="modal-title" id="exampleModalLongTitle">Registros Bodega</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" >
                <div class="card-body">
                    <div class="table-responsive">    
                        <table id="mydatatable_bodega" class="table" style="margin: auto;">
                            <thead class="">
                                <tr>
                                    <th scope="col">Id</th>
                                    <th scope="col">Orden Picking</th>
...
function datosTerminacion(id){
    $('#mydatatable_terminacion').DataTable().destroy().draw();
    var URLdominio = getAbsolutePath();
    var url = URLdominio + "list/consulta/terminacion";
        $.ajax({
            url: url,
            type: 'GET',
            data: {
            ...
            success: function(data){
                $(document).ready(function(){
                    $('#mydatatable_terminacion').DataTable({
                        dom: 'Blfrtip',
                        buttons: [{
                        extend: 'excel',
                        footer: true,
                        title: 'Organización Bless',
                        filename: "Detalle Picking Terminacion Orden: "+data[0]['orden_picking'],
...
function datosBodega(id){
    $('#mydatatable_bodega').DataTable().destroy().draw();
    var URLdominio = getAbsolutePath();
    var url = URLdominio + "list/consulta/bodega";
        $.ajax({
            url: url,
            type: 'GET',
            data: {
