...
<div class="container-fluid mt-2">
    ...
    FORMULARIO DE EDICION PREPARACION
    </div>
        <div class="card-body">
...
<body>
    <form method="" action="" id="terminacionForm" accept-charset="UTF-8" autocomplete="off">
        @csrf
        <input style="display: none" type="number" id="id" name="id" value="{{$pre[0]['id']}}">

        <label for="">Fecha:</label> 
        <input type="date" class="fecha" id="fecha" name="fecha" value="{{$pre[0]['fecha']}}">
        
        <label for="">Turno:</label> 
        <select name="turno" id="turno" class="lista">
            <option value="{{$pre[0]['turno']}}" selected>Turno {{$pre[0]['turno']}}</option>
        ...
        <select name="modulo" id="modulo" class="lista">
            <option value="{{$pre[0]['modulo']}}" selected>{{$pre[0]['modulo']}}</option>
        ...
        <select id="empleado" class="chosen-select" data-placeholder="Seleccione una Empleado" style="width:100%;">
            <option value="" selected disabled></option>
                @foreach($empleados as $empleado)
                    @if($pre[0]['id_empleado'] == $empleado->id)
        ...
        <select id="referencia" class="chosen-select" data-placeholder="Seleccione una Referencia" style="width:100%;" onchange="ShowSelected();">
            ...
            @foreach($referencias as $referencia)
                @if($pre[0]['id_referencia'] == $referencia->id)
        ...
        <input type="text" id="tallas" name="tallas" class="" value="{{$pre[0]['tallas']}}">

        <br>
        <label for="">Tiempo de Ciclo:</label>
        <input type="number" id="tc" name="tc" class="" value="{{$pre[0]['tc']}}">

        <label for="">Tipo:</label>
        <select name="tipo" id="tipo" class="lista">
            <option value="{{$pre[0]['tipo']}}" selected>{{$pre[0]['tipo']}}</option>
        ...
        <select name="hora" id="hora" class="lista">
            <option value="{{$pre[0]['hora']}}" selected>Hora {{$pre[0]['hora']}}</option>
        ...
        <input type="number" id="cantidad" class="" name="cantidad" value="{{$pre[0]['cantidad']}}"> 

        <label for="">Tiempo Requerido en Horas:</label>
        <input type="number" id="tiempo_r" class="" name="tiempo_r" value="{{$pre[0]['tiempo_req_h']}}">
        
        <label for="">Numero de Operarios:</label>
        <input type="number" id="n_operarios" name="n_operarios" value="{{$pre[0]['n_operarios']}}">
        ...
        <select name="tipo_p_prgm" id="tipo_p_prgm">
            ...
            $tipop = ""; 
            for($i=0;$i<count($p);$i++){
                if($p[$i]['id'] == $pre[0]['id_parada_prg']){
                    $tipop = $p[$i]['tipo_parada_prg'];
                }
            }
            ?>
            @if($pre[0]['id_parada_prg']==0)
            <option value="">Seleccione TPP</option>
            @else
            <option value="{{$pre[0]['id_parada_prg']}}">{{$tipop}}</option>
            @endif
            <option value="">Seleccione TPP</option>
        ...
        <select name="tipo_pno_prgm" id="tipo_pno_prgm">
            ...
            for($i=0;$i<count($pn);$i++){
                if($pn[$i]['id'] == $pre[0]['id_parada_noprg']){
                    $tipopn = $pn[$i]['tipo_parada_noprg'];
                }
            }
            ?>
            @if($pre[0]['id_parada_noprg']==0)
            <option value="">Seleccione TPNP</option>
            @else
            <option value="{{$pre[0]['id_parada_noprg']}}">{{$tipopn}}</option>
            @endif
            <option value="">Seleccione TPNP</option>
        ...
        @if($pre[0]['tiempo_noprg']==0)
        <input type="number" id="tiempo_pno_prgm" name="tiempo_pno_prgm" value="">
            @else
            <input type="number" id="tiempo_pno_prgm" name="tiempo_pno_prgm" value="{{$pre[0]['tiempo_noprg']}}">
            @endif
...
<script src="{{asset('js/edit_preparacion.js')}}"></script>
