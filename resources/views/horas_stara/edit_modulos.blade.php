...
<div class="container-fluid mt-2">
    ...
    FORMULARIO DE EDICION CONFECCION STARA
    </div>
...
<body>
    <form method="" action="" id="moduloForm" accept-charset="UTF-8" autocomplete="off">
        @csrf
        <input style="display: none" type="number" id="id" name="id" value="{{$modulo[0]['id']}}">

        <label for="">Fecha:</label> 
        <input type="date" class="fecha" id="fecha" name="fecha" value="{{$modulo[0]['fecha']}}">
        
        <label for="">Turno:</label> 
        <select name="turno" id="turno" class="lista">
            <option value="{{$modulo[0]['turno']}}" selected>Turno {{$modulo[0]['turno']}}</option>
        ...
        <select name="modulo" id="modulo" class="lista">
            ...
            <option value="0">Modulo STARA Pilotos</option>
            <option value="1">Modulo STARA 01</option>
            <option value="2">Modulo STARA 02</option>
            <option value="3">Modulo STARA 03</option>
            <option value="4">Modulo STARA 04</option>
        </select>
        ...
        <select id="referencia" class="chosen-select" data-placeholder="Seleccione una Referencia" style="width:100%;" onchange="ShowSelected();">
            ...
            @foreach($referencias as $referencia)
                @if($modulo[0]['id_referencia']==$referencia->id)
        ...
        <input type="text" id="tallas" value="{{$modulo[0]['tallas']}}">

        <label for="">Tiempo de Ciclo:</label>
        <input type="number" id="tc" name="tc" class="" value="{{$modulo[0]['tc']}}">

        <label for="">Tipo:</label>
        <select name="tipo" id="tipo" class="lista">
            <option value="{{$modulo[0]['tipo']}}" selected>{{$modulo[0]['tipo']}}</option>
        ...
        <select name="hora" id="hora" class="lista">
            <option value="{{$modulo[0]['hora']}}" selected>Hora {{$modulo[0]['hora']}}</option>
        ...
        <input type="number" id="cantidad" class="" name="cantidad" value="{{$modulo[0]['cantidad']}}"> 

        <label for="">Tiempo Requerido en Horas:</label>
        <input type="number" id="tiempo_r" class="" name="tiempo_r" value="{{$modulo[0]['tiempo_req_h']}}">

        <label for="">Numero de Operarios:</label>
        <input type="number" id="n_operarios" class="" name="n_operarios" value="{{$modulo[0]['n_operarios']}}">
        ...
        <select name="tipo_p_prgm" id="tipo_p_prgm">
            ...
            $tipop = ""; 
            for($i=0;$i<count($p);$i++){
                if($p[$i]['id'] == $modulo[0]['id_parada_prg']){
                    $tipop = $p[$i]['tipo_parada_prg'];
                }
            }
            ?>
            @if($modulo[0]['id_parada_prg']==0)
            <option value="">Seleccione TPP</option>
            @else
            <option value="{{$modulo[0]['id_parada_prg']}}">{{$tipop}}</option>
            @endif
            <option value="">Seleccione TPP</option>
        ...
        <select name="tipo_pno_prgm" id="tipo_pno_prgm">
            ...
            for($i=0;$i<count($pn);$i++){
                if($pn[$i]['id'] == $modulo[0]['id_parada_noprg']){
                    $tipopn = $pn[$i]['tipo_parada_noprg'];
                }
            }
            ?>
            @if($modulo[0]['id_parada_noprg']==0)
            <option value="">Seleccione TPNP</option>
            @else
            <option value="{{$modulo[0]['id_parada_noprg']}}">{{$tipopn}}</option>
            @endif
            <option value="">Seleccione TPNP</option>
        ...
        @if($modulo[0]['tiempo_noprg']==0)
        <input type="number" id="tiempo_pno_prgm" name="tiempo_pno_prgm" value="">
            @else
            <input type="number" id="tiempo_pno_prgm" name="tiempo_pno_prgm" value="{{$modulo[0]['tiempo_noprg']}}">
            @endif
        
        
        <br><br>
        <a href="{{route('list_modulos_stara')}}" class="btn btn-secondary" >Cancelar</a>
...
<script src="{{asset('js/edit_modulos_stara.js')}}"></script>
